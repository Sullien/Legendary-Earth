# Unciv-Legendary-Earth
The ultimate Earth Map made for Unciv; Featuring True Start Locations, historical resources and World Wrap. (V.01 = Pre-release Version)

![alt text](https://github.com/Sullien/Legendary-Earth/blob/main/preview.jpg)

Inspired by: Google Maps
NASA Earth Map by Terrain
Earth Map Topographic
Hoi4 Earth Map

Info for Resource Placement:
https://mrdata.usgs.gov/mrds/

Current Roadmap

1:North Moluca no Existo

A couple of islands in the Pacific are not on the map, this is because they are very close together and that is hard to represent as they would be 1 tile apart, I do plan on adding a few more but it will require moving the landmass around

2: Russian Psy-Op Siberia

Relative to the map scale, Siberia is currently way bigger than the rest of the landmasses on the map, this is due to the fact that originally the map was meant to be smaller, I had to resize it because I ran out of space, however that now meant that Siberia was much bigger than it originally was(it's size was relative to other countries). Siberia will go on a diet soon, but for now it will be quite chonky

3: Timmy can't Play

Unciv isn't very well suited for Earth maps, a lot of CIty-States are quite close (Brussels and Antwerp would be 1 tile apart in the map, as would Kuala Lumpur and Singapore, etc) There are also many nations missing such as Australia, Argentina or Kazakhstan which means the map will be a bit empty without their historical nations. I could make mod Nations and City States to fill it out but that would make this more a scenario than an Earth map.

4: Africa pretty 
Currently Africa has way more features and resources than it realistically should, Particularly on the Sahara and North Africa, This is  because a Normal Africa would be quite empty and boring to see. However in the name of realism Africa shall be stripped bare, it's resources and riches taken away so it looks (sadly) more historical 

5: Resource Compendium
What constitues a resource? Many of the fishing hotspots in the past have long since dried up and are no longer used, should they get a Fish resource? Similarly, many Coal, Iron and Gold mines were depleted and no longer have the resource, should they get a resource? Resources don't decay in Unciv While historical, resource placement aligns more with modern data(Oil in Middle East was discovered decades after WW2) and current deposits since they have the most info. I could add more resource deposits from the past but I need a source of information to do it.

6:Resource Mismatch
A few resources have steep placing conditions(For example, Uranium can only be in forest/Jungle Marsh or tundra), therefore I cannot place them in their historical locations (Niger Uranium should be higher up, a few mines on Egypt cannot be placed etc.) I could change or remove these restrictions but for now this is only a map mod not a ruleset change.

Currently Detained City State/Nations
1(Byzantium) - Too Close to Greece and The Ottoman, their capital(Istanbul) is also the Ottoman Capital
2(Celts) = No real defined map location, their capital in Scotland is too close to England
3(The Huns) - Their capital(Attila's Court) has no real map location, allegedly it was near current-day Budapest in Hungary which is a City-State
4(The Maya) - Too close to the Aztecs
5(Antwerp CS) - Too close to the Netherlands
6(Brussels CS) - Too close to the Netherlands
7(Belgrade CS) - Too Close to Budapest
8(Bratislava CS) - Too close to Austria
9(Florence CS) - Too close to Rome
10(Geneva CS) - Too close to Zurich/France
11(Genoa CS) - Too Close to Rome
12(Kuala Lumpur CS) - Too close to Singapore
13(La Venta CS) - Too close to Aztecs
14(Milan CS) -  Too close to Rome
15(Monaco CS) - Too close to Rome
16(Prague CS) - Too close to Austria
17(Sidon CS) - Too close to Jerusalem
18(Tyre CS) - Too close to Jerusalem
19(Vatican CS) - Too Close to Rome
20(Venice CS) - Too Close to Rome/Austria
21(Wittenberg CS) - Too Close to Germany/France
22(Zanzibar CS) - Too Close to Mombasa
23(Zurich CS) - Too Close to Rome,France and Germany
24(Valletta CS) - Funny AI doesn't ever settle

Funny observations: AI doesn't Settle in place! While I have historical start locations, some of the nations move around before settling, Carthage and Arabia love doing it for example
